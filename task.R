...
legend(0.6,0.3,c("LOGIT FULL MODEL","LOGIT AIC MODEL", "LOGIT BIC MODEL"),lty=c(1:3),col=c(1:3),pch=c(1:3))				#为6个模型添加标示，区分6个模型


library(pROC)
pred.full=predict(model.full,data=data_train)
pred.aic=predict(model.aic,data=data_train)
pred.bic=predict(model.bic,data=data_train)

roc.full=roc(data_train$target,pred.full)
roc.aic=roc(data_train$target,pred.aic)
roc.bic=roc(data_train$target,pred.bic)
